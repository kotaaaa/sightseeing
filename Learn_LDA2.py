...
def main():
    ...
    texts=[]
    urls=[]
    print('aumo start reading')
    # df_all = pd.read_csv('../SS_contents/save_df3-3_4.csv')#,names=['idx_in_query','text_flag','query','topicID','url'])
    df_all = pd.read_csv('./trip_site/aumo.txt',names=['url'])#,names=['idx_in_query','text_flag','query','topicID','url'])
    print(df_all.shape)
    print(df_all.head())
    print(df_all.tail())
    for key,row in df_all.iterrows():
        ...
        # print(key)
        # try:
        # f=open('../SS_contents/text3-3/'+str(df_all.at[df_all.index[key],'queryID'])+'/'+str(key)+'_'+str(df_all.at[df_all.index[key],'queryID'])+'_text.csv')
        f=open('./trip_site/text_aumo/'+str(key)+'_text.txt')
    ...
    for num_topic in num_topics:
        ...
        id2word.save_as_text('dict_aumo.txt')
        id2word = gensim.corpora.Dictionary.load_from_text('dict_aumo.txt')

        gensim.corpora.MmCorpus.serialize('cop_aumo.mm', corpus)
        corpus = gensim.corpora.MmCorpus('cop_aumo.mm')

        lda.save('lda_aumo.model')
        lda = gensim.models.ldamodel.LdaModel.load('lda_aumo.model')

        df = get_lda_topics(lda, num_topic)
        # print(df)
        df.to_csv('LDA_words_'+str(num_topic)+'_aumo.csv')
        cohsum=0
        ...
        data.to_csv('LDA_'+str(num_topic)+'_aumo.csv',header=None)
        print(data.head())
        print(data.tail())
        print(data.shape)
