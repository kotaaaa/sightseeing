...
for i in range(803):
    ...
    list_citys = os.listdir('./Ariticles_citys/')
    if not str(i) in list_citys:
        os.mkdir('./Ariticles_citys/'+str(i))

    df = pd.read_table('./Citys_URL/Date_Url_'+str(i)+'.txt',names=['Date','Url','Title'])
    ...
    for key,row in df.iterrows():
        ...
        for j in range(10):
            try:
                ...
                break
        # exit()
        with open('./Ariticles_citys/'+str(i)+'/'+str(key)+'article_'+str(i)+'.txt','w') as fw:
            fw.write(article)
        # for i in texts:
        #     # print(i.prettify())
        #     # print(i)
